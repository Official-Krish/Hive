...
| `watchdog.check`            | `WATCHDOG_INTERVAL_MS` (1m) | Proactive alerts: stuck agents, token burn, test streaks, budget risk |
...
| `WATCHDOG_INTERVAL_MS`     | `60000`   | Watchdog alert scan cadence                              |
| `WATCHDOG_STUCK_MIN`       | `10`      | Stuck-session WARNING threshold (min; CRITICAL at 3x)    |
| `WATCHDOG_BURN_TOKENS`     | `200000`  | Token-burn flag threshold (tokens/60m, zero file output) |
| `WATCHDOG_BURN_COST_CENTS` | `500`     | Token-burn CRITICAL cost threshold (cents)               |
