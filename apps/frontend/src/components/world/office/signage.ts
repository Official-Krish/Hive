...
/** Reception brand lockup: glowing hex mark + HIVE wordmark + tagline.
 *  Transparent background — the dark slate feature wall shows through.
 *  1024×256 canvas maps to a 2.6×0.65m panel without stretch. Faces the
 *  lobby (mount with rotation [0, Math.PI, 0] so it reads correctly). */
export function receptionTexture(): THREE.CanvasTexture {
  const key = "reception:hive-v1";
  const hit = cache.get(key);
  if (hit) return hit;
  const [el, ctx] = canvas(1024, 256);
  ctx.clearRect(0, 0, 1024, 256);
  const glow = "#38bdf8";
  // hex mark (echoes the HexLogo meshes above the video wall)
  const cx = 128;
  const cy = 128;
  const r = 84;
  ...
  ctx.fillText("HIVE", 268, 148);
  // tagline rule + line
  ctx.fillStyle = glow;
  ctx.fillRect(272, 172, 120, 6);
  ctx.fillStyle = "#8b93a5";
  ctx.font = "500 34px system-ui, -apple-system, 'Segoe UI', sans-serif";
  ctx.fillText("ENGINEERING INTELLIGENCE", 272, 222);
  const tex = toTexture(el);
  cache.set(key, tex);
  return tex;
}
