...
export function CallControls({
  ...
  const btn = (on: boolean) =>
    cn(
      "grid size-9 place-items-center rounded-full transition-colors focus-visible:outline-2 focus-visible:outline-neutral-900/50",
      on
        ? "bg-neutral-950 text-white hover:bg-neutral-800"
        : "bg-rose-500 text-white hover:bg-rose-600",
    );
  const press = reduce ? undefined : { scale: 0.9 };
  return (
    <div
      role="toolbar"
